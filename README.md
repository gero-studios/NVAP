...
Metrics are physically calibrated using voxel spacing in micrometres. The
default spacing is:
...
**AMD GPUs on Windows:** the DirectML wheel is published only for Python ≤ 3.12,
so a Python 3.13 environment runs on CPU even when the GPU is present. A
project-local Python 3.12 setup:
...
python -m pip install -e ".[dev,directml]"
nvap --debug
```
...
1. **Default thresholds** (Green 0.80, Red 0.60) are applied.
...
**Wipe Specks** manually at any time.
